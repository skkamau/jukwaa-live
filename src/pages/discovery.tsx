  ...
  return <div className="page">
    <section className="hero">
      <div className="hero-copy">
        <span className="eyebrow">FICTIONAL DEMO · THE 254</span>
        <h1>Kenya is<br /><em>live.</em></h1>
        <p>Watch the stories, sounds, games and conversations shaping the culture.</p>
        <div className="hero-actions"><Link className="btn btn-white" to={`/watch/${featured.id}`}><Play /> Watch demo</Link><FollowButton creatorId={creator.id} /></div>
        <div className="hero-creator"><span className="verified">✓</span><b>{creator.name}</b><span>{featured.category}</span></div>
      </div>
      <Link className="hero-preview" to={`/watch/${featured.id}`} style={{ "--accent": featured.accent } as React.CSSProperties}>
        <LiveBadge /><ViewerCount count={featured.viewers} /><div className="signal"><i /><i /><i /><i /></div>
        <div className="hero-preview-info"><span>{featured.title}</span><small>{featured.duration} demo live</small></div>
      </Link>
    </section>
    <Section title="Demo live now" link="/browse">
      <span className="content-source-label">FICTIONAL DEMO STREAMS</span>
      <div className="stream-grid">{streams.slice(0, 8).map(stream => <StreamCard key={stream.id} stream={stream} />)}</div>
    </Section>
    <Section title="Real live now" link="/browse">
      {real.loading ? <p className="content-loading">Checking live channels…</p> : real.streams.length ? <div className="stream-grid">{real.streams.slice(0, 8).map(stream => <RealStreamCard key={stream.id} stream={stream} />)}</div> : <EmptyState title="No real streams are live" text={real.unavailable ? "The API is unavailable. Start the backend to load real streams." : "Prepared and ended streams do not appear in public live discovery."} />}
    </Section>
    <Section title="Demo categories" link="/browse#categories"><div className="category-grid">{categoryMeta.map(category => <CategoryCard key={category.name} {...category} />)}</div></Section>
    <Section title="Demo events"><div className="event-grid">{events.map(([name, date, time]) => <article className="event" key={name}><CalendarDays /><div><span>{date} · {time}</span><h3>{name}</h3></div><button onClick={() => toast(`Demo reminder set for ${name}`)}><Bell /> Remind me</button></article>)}</div></Section>
  </div>;
}
